...
func (m *MPointCalcrator) CalcratePoint(player *MPlayer, agarikei *CountOfShantenAndAgarikei, table *MTable, yakus MYakus) *MPoint {
	// ˅
	point := &MPoint{}
				...
				for tileID, tileCount := range mentsus[i] {
					if tileCount == 2 {
						if tileID == 34 ||
							tileID == 35 ||
							tileID == 36 ||
							((tileID == 31 && *table.GetStatus().Kaze == Kaze_KAZE_TON) || (tileID == 31 && *player.status.Kaze == Kaze_KAZE_TON)) ||
							((tileID == 31 && *table.GetStatus().Kaze == Kaze_KAZE_NAN) || (tileID == 31 && *player.status.Kaze == Kaze_KAZE_NAN)) ||
							((tileID == 31 && *table.GetStatus().Kaze == Kaze_KAZE_SHA) || (tileID == 31 && *player.status.Kaze == Kaze_KAZE_SHA)) ||
							((tileID == 31 && *table.GetStatus().Kaze == Kaze_KAZE_PE) || (tileID == 31 && *player.status.Kaze == Kaze_KAZE_PE)) {
							point.Hu += 2
						}
					}
				}
