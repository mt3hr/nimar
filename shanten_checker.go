...
func (s *ShantenChecker) preparation(player *MPlayer) {
	...
	for _, openedTiles := range []*OpenedTiles{
		...
		if openedTiles != nil && len(openedTiles.Tiles.Tiles) != 0 {
			switch openedTiles.OpenType {
			case OpenType_OPEN_PON:
				minko := TileIDs{}
				for _, tile := range openedTiles.Tiles.Tiles {
